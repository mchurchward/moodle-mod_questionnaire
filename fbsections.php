...
if (data_submitted()) {
    ...
    foreach($vf as $key => $value){
        ...
        // $qid_section[1] = qid
        // $qid_section[2] = section
        $scorecalculation_weights[$qid_section[1]][$qid_section[2]] = $value;
    }
    foreach ($vf as $qs) {
        $sectionqid = explode("_", $qs);
        if ($sectionqid[0] != 0) {
            if ($advdependencies && isset($sectionqid[1]))
                $scorecalculation[$sectionqid[0]][$sectionqid[1]] = $scorecalculation_weights[$sectionqid[1]][$sectionqid[0]];//null;
            else
    ...
    if ($c < $feedbacksections) {
        ...
        for ($section = 1; $section <= $feedbacksections; $section++) {
            if ($existsection = $DB->get_record('questionnaire_fb_sections',
                ...
                $value = serialize($scorecalculation[$section]);
                $DB->set_field('questionnaire_fb_sections', 'scorecalculation', $value,
    ...
    if (!$cannotuse) {
        ...
        if ($qhasvalues) {
            ...
            for ($i = 0; $i < $feedbacksections; $i++) {
                ...
                if ($i != 0) {
                    if ($advdependencies) {
                        $output .= '<div class="' . $bg . '"><input type="checkbox" name="' . $n . '_' . $i . '" id="' .
                    ...
                    if ($advdependencies) {
                        $output .= '<div class="' . $bg . '"><input type="checkbox" name="' . $n . '_' . $i . '" id="' . $i . '" value="' . $i . '"';
                    } else{
